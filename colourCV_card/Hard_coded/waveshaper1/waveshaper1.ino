...
void i2s0_rx_isr(void) {//this ISR is not used when in sync mode. if it is enabled then audio has glitches
  ...
  right_sampleIn = I2S0_RDR0;       // dummy read
  right_sampleIn  = I2S0_RDR0;       // read channel 1
...
void FASTRUN i2s0_tx_isr(void) {
  ...
  ramp++;
  //if (ramp > 48000)ramp = 26000;

  //left_sampleOut = ((left_sampleIn * reading12)>>13); 
  left_sampleOut = distoTable[ramp>>7];  


  // if (I2S0_RCSR & I2S_RCSR_FEF) I2S0_RCSR |= I2S_RCSR_FEF; // clear if underrun may not be needed
  // if (I2S0_RCSR & I2S_RCSR_SEF) I2S0_RCSR |= I2S_RCSR_SEF; // clear if frame sync error may not be needed, test more
}
